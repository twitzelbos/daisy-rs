...
// Maximum length of control transfer data stage in bytes. Upstream is 128 (256
// with the "control-buffer-256" feature). daisy-rs PATCH: the whole config
// descriptor is assembled into this buffer before EP0 sends it, so it caps the
// config-descriptor size. Our CDC + UAC1 + MIDI composite is ~369 bytes, which
// overflows even 256 and silently stalls GET_DESCRIPTOR(config) — the device
// enumerates its device descriptor, gets an address, then the host can never
// read the configuration and drops it. 512 fits the composite with headroom.
// See vendor/usb-device/PATCH.md.
