...
impl<USB: UsbPeripheral> usb_device::bus::UsbBus for UsbBus<USB> {
    ...
    fn enable(&mut self) {
        ...
        interrupt::free(|cs| {
            ...
            // daisy-rs local patch: in the `hs` branch we deliberately do
            // NOT set FDMOD here. Upstream writes FDMOD before CSRST, but
            // per RM0433 §57.14.5 the core soft reset clears every CSR
            // except AHB config back to reset — including GUSBCFG.FDMOD.
            // The pre-reset write is therefore wiped and the peripheral
            // ends up in ID-pin-driven mode after the reset, which on the
            // Daisy Seed (ID floating, host-preferred) leaves the SIE in
            // an inconsistent state where USBSUSP fires but D+ pull-up
            // never engages — no enumeration. Set FDMOD after CSRST
            // instead, further down, matching ST HAL_PCD_Init ordering.
            // We still set SRPCAP=0 and TOCAL up front because those
            // aren't touched by CSRST for our AHB configuration.
            #[cfg(feature = "hs")]
            modify_reg!(otg_global, regs.global(), GUSBCFG,
                SRPCAP: 0,
                TOCAL: 0x1
            ...
            // daisy-rs local patch: force device mode AFTER the core soft
            // reset (CSRST clears GUSBCFG.FDMOD, so a pre-reset write is
            // wiped). RM0433 §57.14.4: "After setting the force bit, the
            // application must wait at least 25 ms before the change
            // takes effect." ST HAL then polls GINTSTS.CMOD (bit 0, 0 =
            // device, 1 = host) until it flips. See ST's
            // USB_SetCurrentMode at stm32h7xx_hal_driver/Src/
            // stm32h7xx_ll_usb.c:254-291.
            #[cfg(feature = "hs")]
            {
                modify_reg!(otg_global, regs.global(), GUSBCFG, FDMOD: 1);
                #[cfg(feature = "cortex-m")]
                {
                    // 25 ms guarantee at up to 480 MHz sysclk (longer at
                    // lower sysclks; harmless).
                    cortex_m::asm::delay(12_000_000);
                    // Then a bounded readback poll for up to another
                    // ~50 ms in case the mode switch is slow.
                    let mut budget: u32 = 24_000_000;
