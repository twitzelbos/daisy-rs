*** Settings ***
Documentation    Boot the daisy-usb-audio app (built with `renode_test`, which
...              skips the USB bring-up sim can't model) from QSPI XIP at
...              0x9000_0000, exactly as the bootloader would. If PC7 toggles,
...              the app's Reset handler + #[pre_init] (MPU/caches/DWT) + main
...              are executing from FLASH — proving the XIP conversion is sound.
Suite Setup      Setup
...
*** Test Cases ***
USB Audio App Boots From XIP
    Execute Command    mach create "daisy-usb-audio"
    Execute Command    machine LoadPlatformDescription @${PLATFORM}
    # QSPI XIP window (0x9000_0000) served by the patched controller + flash.
